...
@section('content')
    ...
    <section class="w-auto p-5 mx-5 my-2 rounded-md shadow-md" style="height: 75vh">
        <h2 class="text-lg font-semibold  capitalize ">Edit Role</h2>

        <form action="/dashboard/role/edit-role/{{ $role->name }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 gap-3 mt-2">
                    ...
                    @foreach ($permissions as $item)
                        <div class="flex items-center mt-2">
                            <input type="checkbox" id="{{ $item->name }}" name="permissions[]" value="{{ $item->name }}"
                                ...
                                @if (in_array($item->name, old('permissions', $role->permissions->pluck('name')->toArray()))) checked @endif>
                            <label for="{{ $item->name }}" class="ml-2 text-sm">{{ $item->name }}</label>
                ...
                <div class="relative text-left">
                    ...
                    <div>
                        <div id="selected-options-container" class="flex flex-wrap" value="">
                            @foreach ($role->permissions as $item)
                                <span class="bg-gray-200 text-gray-800 px-2 py-1 rounded mr-2 mb-2 flex items-center">
                                    {{ $item->name }}
                                    <button type="button" onclick="showSelectedOptions()"
                                        class="ml-2 text-sm font-bold focus:outline-none">x</button>
                                </span>
                            @endforeach
                        </div>
                        ...
                        <div class="">
                            <div class="flex items-center text-center">
                                ...
                                <label for="select-all"
                                    class="text-sm font-medium w-full p-2 text-left text-grey-700 bg-white-100 border-gray-300 option-label hover:bg-gray-200">Select
                                    All</label>
                            </div>
                            @foreach ($permissions as $item)
                                <div class="flex items-center text-center">
